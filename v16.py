...
# Helper functions for Search and Visual Builder
def get_mock_events():
    return [
        {"date": "2024-03-01", "type": "Filing", "desc": "Q4 Earnings Report"},
        {"date": "2023-12-15", "type": "News", "desc": "Product Launch in EMEA"},
        {"date": "2023-11-20", "type": "M&A", "desc": "Acquired smaller competitor"},
    ]


def render_hierarchy(entity):
    fig = go.Figure(
        go.Icicle(
            ids=["Root", "Parent", entity["company_id"]],
            labels=[
                "Global Holdings",
                entity["parent_id"] or "Parent Co",
                entity["company_name"],
            ],
            parents=["", "Root", "Parent"],
        )
...
def page_visual_builder():
    ...
    with left_col:
        ...
        for idx, (title, subtitle) in enumerate(steps):
            cls = (
                "active vb-step"
                if idx == st.session_state.vb_step
                else "inactive vb-step"
            )
            ...
            st.markdown(
                f"<div class='{cls}'><div class='label'>{title}</div><div style='font-size:0.8rem;color:var(--muted-color)'>{subtitle}</div></div>",
                unsafe_allow_html=True,
            )
    ...
    def render_data_sources():
        ...
        st.markdown(
            "Select a primary dataset and optionally upload your own CSV/Excel file."
        )
        ds_keys = list(source_map.keys())
        primary_ds = st.selectbox(
            "Primary dataset", ds_keys, index=0, key="vb_primary_ds"
        )
        ...
        uploaded_file = st.file_uploader(
            "Or upload your own dataset (CSV/Excel)",
            type=["csv", "xlsx"],
            key="vb_upload",
        )
        if uploaded_file:
    ...
    def render_joins():
        ...
        if cached is None:
            st.info(
                "No working dataset pinned. Select primary dataset in 'Data Sources' or click 'Use this as builder's working dataset'."
            )
        working_df = (
            cached.copy()
            if cached is not None
            else source_map[list(source_map.keys())[0]].copy()
        )

        st.caption(f"Working rows: {len(working_df)} • cols: {len(working_df.columns)}")
        all_ds = list(source_map.keys())
        other_ds = st.selectbox(
            "Select dataset to inspect for joins",
            [d for d in all_ds],
            index=1 if len(all_ds) > 1 else 0,
        )
        ...
        with st.form("vb_join_form", clear_on_submit=True):
            other_ds_sel = st.selectbox(
                "Join with dataset",
                [
                    d
                    for d in all_ds
                    if d != st.session_state.get("vb_primary_ds", all_ds[0])
                ],
            )
            right_df2 = source_map.get(other_ds_sel, pd.DataFrame())
            left_key = st.selectbox(
                "Primary key (left)", ["--select--"] + safe_cols(working_df)
            )
            right_key = st.selectbox(
                "Other key (right)", ["--select--"] + safe_cols(right_df2)
            )
            how = st.selectbox("Join type", ["inner", "left", "right", "outer"])
            add_join = st.form_submit_button("Add Join")
            if add_join:
                if left_key in ("--select--", None) or right_key in (
                    "--select--",
                    None,
                ):
        ...
        if st.session_state.vb_join_steps:
            ...
            for i, j in enumerate(st.session_state.vb_join_steps):
                ...
                cols[0].markdown(
                    f"`{i + 1}.` {j['left_on']} ⟷ {j['other_ds']}.{j['right_on']} ({j['how']})"
                )
                ...
                if (
                    cols[2].button("Down", key=f"join_down_{i}")
            ...
            try:
                ...
                for s in st.session_state.vb_join_steps:
                    ...
                    preview = preview.merge(
                        ...
                        right_on=s["right_on"],
                        how=s["how"],
                        suffixes=("", f"_{s['other_ds'].replace(' ', '_')}"),
                    )
    ...
    def render_refine():
        ...
        if working_df.empty:
            st.info(
                "No working dataset. Build or pin one on the Data Sources / Joins step."
            )
            return
        ...
        with st.form("vb_add_qgroup", clear_on_submit=True):
            ...
            if col != "--select--":
                if pd.api.types.is_numeric_dtype(refined[col]):
                    op = st.selectbox(
                        "Operator", ["==", "!=", ">", "<", ">=", "<="], key="gop_num"
                    )
                    val = st.number_input("Value", key="gop_num_val")
                else:
                    op = st.selectbox(
                        "Operator", ["contains", "==", "!="], key="gop_str"
                    )
            ...
            if addg:
                if col == "--select--":
                    ...
                    st.session_state["vb_query_groups"].append(
                        {"op": gop, "rules": [{"col": col, "op": op, "val": val}]}
                    )
    ...
    def render_transform():
        ...
        base_df = st.session_state.get(
            "_vb_refined_df",
            st.session_state.get("_vb_cached_working_df", pd.DataFrame()),
        )
        if base_df.empty:
        ...
        with st.form("vb_calc_form", clear_on_submit=True):
            ...
            expr = st.text_area(
                "Expression (pandas syntax, use `df` as input, e.g. df['amount']-df['cost'])",
                key="tf_expr",
            )
        ...
        with st.form("vb_type_form", clear_on_submit=True):
            ...
            colt = st.selectbox(
                "Column to change type", ["--select--"] + cols, key="tf_col_select"
            )
            tgt = st.selectbox(
                "Target type", ["int", "float", "str", "datetime"], key="tf_type_select"
            )
    ...
    def render_aggregate():
        ...
        with st.form("vb_add_agg", clear_on_submit=True):
            numeric_choices = [
                c for c in cols_avail if pd.api.types.is_numeric_dtype(df_for_agg[c])
            ]
            sel = st.selectbox(
                "Numeric column", ["--select--"] + numeric_choices, key="vb_agg_sel"
            )
    ...
    def render_charts():
        ...
        chart_kind = st.selectbox(
            "Chart type", ["Bar", "Line", "Scatter", "Area", "Box", "Pie"], index=0
        )
        ...
        if st.button("Build Chart"):
            try:
                ...
                if chart_kind == "Bar":
                    fig = px.bar(
                        src_df,
                        x=None if x_axis == "--select--" else x_axis,
                        y=None if y_axis == "--select--" else y_axis,
                        **kwargs,
                        title=title,
                        template="plotly_dark",
                    )
                    ...
                    fig = px.pie(
                        src_df,
                        names=None if x_axis == "--select--" else x_axis,
                        values=None if y_axis == "--select--" else y_axis,
                        title=title,
                        template="plotly_dark",
                    )
                if (
                    add_trend
                    and chart_kind in ("Scatter", "Line", "Bar")
                    and x_axis != "--select--"
                    and y_axis != "--select--"
                ):
                    try:
    ...
    def render_composer():
        ...
        with c1:
            if st.button("Export Composer → PDF"):
                # Use basic generator here; user can hook up build_pdf_from_composer if present
                st.warning("Integration with full PDF composer pending.")
        with c2:
            st.button("Export Composer → PPTX", disabled=True)
        with c3:
            st.button("Email Composer (attach PDF)", disabled=True)

    def render_scheduler():
        st.markdown("## 8. Scheduler — Automate Exports")
        defaults = st.session_state.get("scheduler_defaults", {})
        job_name = st.text_input(
            "Job Name", defaults.get("name", "Weekly Builder Report")
        )
        ...
        recipients = st.text_area(
            ...
            defaults.get("recipients", "admin@company.com"),
        )
        ...
        if "scheduled_jobs" in st.session_state and st.session_state.scheduled_jobs:
            st.dataframe(
                pd.DataFrame(st.session_state.scheduled_jobs)[
                    ["name", "source", "freq", "status", "next_run"]
                ],
                use_container_width=True,
            )
        else:
    ...
    with main_col:
        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        st.markdown(
            f"### Step {st.session_state.vb_step + 1} — {steps[st.session_state.vb_step][0]}"
        )
        # call renderer
        try:
        ...
        with nav_cols[2]:
            if st.button("Save View", use_container_width=True):
                name = st.text_input(
                    "Save view as",
                    value=f"View_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    key="vb_save_name",
                )
...
def page_search():
    """
    Updated Search Page:
    - Robust multi-term + fuzzy search (difflib fallback)
    - Handles missing columns like tax_number gracefully
    - Multi-select results, side-by-side comparison, timeline, hierarchy
    - PDF export for selected companies using existing PDFEngine
    """
    st.title("Global Registry Search")
    st.caption(
        "Search multiple companies at once and view detailed corporate intelligence."
    )

    df = st.session_state.get("company_df", pd.DataFrame())
    if df is None or df.empty:
        st.warning(
            "Company registry is empty. Go to Data Studio or regenerate company registry."
        )
        return

    # Prepare safe columns
    if "tax_number" not in df.columns:
        df["tax_number"] = ""  # harmless placeholder

    query = st.text_input(
        "Search Companies",
        placeholder="Stark, Wayne, COMP-1001, part-of-name…",
        help="You can enter multiple comma-separated terms.",
    )

    # Fuzzy helper function (prefer rapidfuzz if available)
    def fuzzy_match(series, term):
        try:
            from rapidfuzz import fuzz

            scores = series.astype(str).apply(
                lambda s: fuzz.partial_ratio(s.lower(), term.lower())
            )
            return scores >= 70
        except Exception:
            # fallback to difflib approximate matching on unique values
            uniq = series.dropna().astype(str).unique().tolist()
            close = set()
            try:
                matches = difflib.get_close_matches(term, uniq, n=50, cutoff=0.6)
                close.update(matches)
            except Exception:
                pass
            mask = series.astype(str).apply(
                lambda s: any(c.lower() in s.lower() for c in close)
                or term.lower() in str(s).lower()
            )
            return mask

    if not query:
        st.info("Type a name, ID or keyword to begin search.")
        return
    ...
    for t in terms:
        # search across common columns safely
        ...
        # Safe strict check for columns
        valid_cols = [c for c in searchable_cols if c in df.columns]

        for col in valid_cols:
            # check if column is actually present in this iteration to be paranoid
            if col not in df.columns:
                continue

            # exact-ish contains OR fuzzy
            try:
                # safe string conversion
                s_col = df[col].astype(str).fillna("")
                contains_mask = s_col.str.contains(t, case=False, na=False)
                fuzzy_mask = fuzzy_match(df[col], t)
                local_mask |= contains_mask | fuzzy_mask
            except Exception:
                # fallback if column access fails
                continue

        # also try numeric/fuzzy on revenue_mm or employees
        if "revenue_mm" in df.columns and t.replace(".", "", 1).isdigit():
            val = float(t)
            # Safe numeric compare
            revenue_s = df["revenue_mm"].fillna(0)
            local_mask |= (revenue_s >= val * 0.9) & (revenue_s <= val * 1.1)
        mask |= local_mask
    ...
    # Compact table of results with quick facets
    cols_to_show = [
        c
        for c in [
            "company_id",
    ...
    st.dataframe(
        results[cols_to_show].sort_values(by=cols_to_show[0]).reset_index(drop=True),
        use_container_width=True,
        height=260,
    )

    # Multi-select for comparison / details
    selected_ids = st.multiselect(
        "Select companies to inspect/compare",
        options=results["company_id"].tolist(),
        default=[results["company_id"].iloc[0]],
        format_func=lambda cid: f"{cid} — {df.loc[df['company_id'] == cid, 'company_name'].iloc[0]}",
    )

    if not selected_ids:
        st.info("Select at least one company to view details.")
        return

    # If single selected: show detailed profile; if multiple: show comparison grid
    selected_rows = df[df["company_id"].isin(selected_ids)].copy()

    if len(selected_rows) == 1:
        entity = selected_rows.iloc[0]
        st.markdown("---")
        ThemeEngine.render_header(
            entity.get("company_name", "Company"), subtitle=entity.get("sector", "")
        )
        st.caption(entity.get("company_id", ""))

        # KPIs row
        k1, k2, k3, k4 = st.columns(4)
        k1.metric("Revenue (MM €)", f"{entity.get('revenue_mm', 0):,.1f}")
        k2.metric("Employees", f"{entity.get('employees', 0):,}")
        k3.metric("Founded", entity.get("founded_year", "n/a"))
        k4.metric("Risk Score", f"{entity.get('risk_score', 'n/a')}/100")

        # Tabs: Timeline, Hierarchy, Financials
        t1, t2, t3 = st.tabs(["Timeline", "Hierarchy", "Financials"])
        with t1:
            st.markdown("#### Timeline")
            # generate a deterministic pseudo-timeline for demo using hash
            base_events = [
                ("2024-01-15", "Filing", "Annual Report filed"),
                ("2023-11-20", "Acquisition", "Acquired a tech subsidiary"),
                ("2023-05-10", "Legal", "Settled litigation"),
            ]
            for d, typ, note in base_events:
                st.write(f"**{d} — {typ}** — {note}")
            st.info(
                "For real data: wire in your events table (e.g. filings, press, M&A)."
            )

        with t2:
            st.markdown("#### Hierarchy")
            st.write("Parent:", entity.get("parent_id") or "—")
            st.write("Subsidiaries Count:", int(entity.get("subsidiaries_count", 0)))
            st.info("Hierarchy chart placeholder — can integrate D3/tree visual.")

        with t3:
            st.markdown("#### Financials")
            # small synthetic trend (or swap for real data if available)
            vals = np.linspace(0, 1, 12) * entity.get("revenue_mm", 1)
            st.line_chart(vals)

        # Export single company PDF
        if st.button("Export this company profile to PDF"):
            html = f"<h1>{entity.get('company_name', '')}</h1><p>ID: {entity.get('company_id', '')}</p><p>Sector: {entity.get('sector', '')}</p>"
            html += "<h3>KPIs</h3><ul>"
            html += f"<li>Revenue (MM €): {entity.get('revenue_mm', '')}</li>"
            html += f"<li>Employees: {entity.get('employees', '')}</li>"
            html += f"<li>Risk Score: {entity.get('risk_score', '')}</li>"
            html += "</ul>"
            success, out = PDFEngine.generate(
                html, f"profile_{entity.get('company_id', '')}.pdf"
            )
            if success:
                st.download_button(
                    "⬇ Download Profile PDF",
                    out,
                    file_name=f"profile_{entity.get('company_id', '')}.pdf",
                    mime="application/pdf",
                )
            else:
                st.error("PDF generation failed.")
        ...
        comp_df = selected_rows[comp_cols].set_index("company_id")
        st.dataframe(comp_df, use_container_width=True, height=240)
        ...
        for idx, row in selected_rows.iterrows():
            with st.container():
                st.markdown(
                    f"**{row.get('company_name', '')}** — {row.get('company_id', '')}"
                )
                cols = st.columns(3)
                cols[0].metric("Revenue (MM €)", f"{row.get('revenue_mm', 0):,.1f}")
                cols[1].metric("Employees", f"{row.get('employees', 0):,}")
                cols[2].metric("Risk", f"{row.get('risk_score', 'n/a')}/100")

        # Simple comparative chart: revenue vs employees
        if "revenue_mm" in df.columns and "employees" in df.columns:
            st.markdown("#### Revenue vs Employees (comparison)")
            fig = go.Figure()
            fig.add_trace(
                go.Bar(
                    x=selected_rows["company_name"],
                    y=selected_rows["revenue_mm"],
                    name="Revenue (MM)",
                )
            )
            fig.add_trace(
        ...
        # Export selected set to PDF
        if st.button("Export selected set to PDF"):
            html = "<h1>Company Comparison</h1>"
            for _, r in selected_rows.iterrows():
                html += (
                    f"<h2>{r.get('company_name', '')} ({r.get('company_id', '')})</h2>"
                )
                html += f"<p>Sector: {r.get('sector', '')}</p><p>Revenue (MM €): {r.get('revenue_mm', '')}</p><p>Employees: {r.get('employees', '')}</p><hr/>"
            success, out = PDFEngine.generate(
                html, f"comparison_{uuid.uuid4().hex[:6]}.pdf"
            )
            if success:
                st.download_button(
...
def page_scheduler():
    ...
    with c1:
        ...
        source_options = ["Composer Report", "Saved View", "Data Dump"]
        def_source_idx = 0
        if defaults.get("source") in source_options:
            def_source_idx = source_options.index(defaults.get("source"))

        source_type = st.radio("Source Type", source_options, index=def_source_idx)

        selected_view = None
        if source_type == "Saved View":
            if st.session_state.get("saved_views"):
        ...
        elif source_type == "Composer Report":
            if not st.session_state.get("my_composer"):
                st.caption("Warning: Composer is currently empty.")
        ...
        format_type = st.radio(
            "Format", ["PDF Report", "Excel Dump", "JSON API Payload"], index=0
        )
    ...
    with c2:
        ...
        if "scheduled_jobs" in st.session_state and st.session_state.scheduled_jobs:
            ...
            cols_to_show = [
                c
                for c in ["name", "source", "freq", "format", "status", "next_run"]
                if c in jobs_df.columns
            ]
            st.dataframe(
                jobs_df[cols_to_show],
                use_container_width=True,
                hide_index=True,
            )
        else:
