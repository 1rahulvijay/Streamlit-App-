...
def page_visual_builder():
    """
    Enhanced Visual Builder with:
      - Multi-dataset join (chain of joins)
      - Multi-column aggregation (multiple agg funcs per column)
      - Chart builder (multiple chart types)
      - Composer: add many charts -> Export combined PDF (section header + description + image)
      - Save / Load Views, lightweight scheduler handoff, email hook
    """
    ...
    # Map datasets (safe fallback to empty DataFrame)
    source_map = {
    ...
    # -----------------------
    # Sidebar: Saved Views & Composer quick info
    # -----------------------
    loaded_config = None
    with st.sidebar:
        ...
        if st.session_state["saved_views"]:
            ...
            if chosen != "Select...":
                loaded_config = st.session_state["saved_views"][chosen]
                st.success(f"Loaded '{chosen}' (applies to UI defaults)")
        else:
            st.info("No saved views yet.")
        st.markdown("---")
        st.caption(f"Composer items: {len(st.session_state['my_charts'])}")
    ...
    # -----------------------
    # Main: Dataset Selection + Join Builder
    # -----------------------
    st.markdown(
        "<div class='glass-card' style='margin-bottom:12px'>", unsafe_allow_html=True
    )
    st.markdown("### 1. Data Source & Join Builder")

    # Primary dataset selection
    ds_cols = list(source_map.keys())
    default_primary = (
        (loaded_config.get("dataset") if loaded_config else ds_cols[0])
        if ds_cols
        else None
    )
    primary = st.selectbox(
        "Primary Dataset", ds_cols, index=get_idx(ds_cols, default_primary)
    )
    primary_df = source_map.get(primary, pd.DataFrame()).copy()

    st.caption(
        f"Primary: {primary} — Rows: {len(primary_df)} | Columns: {len(primary_df.columns)}"
    )

    # Join Steps Editor: allow adding multiple joins in order
    st.markdown("#### Add joins (chain multiple datasets together)")
    if "vb_join_steps" not in st.session_state:
        st.session_state[
            "vb_join_steps"
        ] = []  # list of dicts: {other_ds, left_on, right_on, how}

    # Show existing join steps with ability to remove
    if st.session_state["vb_join_steps"]:
        for i, step in enumerate(st.session_state["vb_join_steps"]):
            cols = st.columns([4, 1])
            with cols[0]:
                st.markdown(
                    f"**Step {i + 1}:** Join {step['other_ds']} (on {step['left_on']} ⟷ {step['right_on']}) — {step['how']}"
                )
            with cols[1]:
                if st.button("Remove", key=f"vb_join_remove_{i}"):
                    st.session_state["vb_join_steps"].pop(i)
                    st.experimental_rerun()

    # Add new join step
    with st.expander("➕ Add Join Step", expanded=False):
        other_ds = st.selectbox(
            "Join With Dataset", [d for d in ds_cols if d != primary]
        )
        other_df = source_map.get(other_ds, pd.DataFrame())
        left_cols = safe_cols(primary_df)
        right_cols = safe_cols(other_df)
        st.markdown("Choose join columns:")
        left_on = st.selectbox(
            "Primary (left) key", ["--select--"] + left_cols, key="vb_left_on"
        )
        right_on = st.selectbox(
            "Other (right) key", ["--select--"] + right_cols, key="vb_right_on"
        )
        how = st.selectbox(
            "Join Type", ["inner", "left", "right", "outer"], index=0, key="vb_join_how"
        )
        if st.button("Add Join Step"):
            if left_on in left_cols and right_on in right_cols:
                ...
                st.error("Please select valid join keys.")

    # Execute the join chain to produce working_df
    working_df = primary_df.copy()
    join_errors = []
    for step in st.session_state["vb_join_steps"]:
        right_df = source_map.get(step["other_ds"], pd.DataFrame()).copy()
        try:
            # Use pandas.merge; allow overlapping column name suffixes
    ...
    if join_errors:
        st.error("Join errors: " + "; ".join(join_errors))

    st.markdown("---")
    st.caption(
        f"Joined dataset rows: {len(working_df)} | cols: {len(working_df.columns)}"
    )
    st.dataframe(working_df.head(30), use_container_width=True, height=200)

    st.markdown("</div>", unsafe_allow_html=True)

    # -----------------------
    # 2. Refinement: Select & Filter
    # -----------------------
    st.markdown(
        "<div class='glass-card' style='margin-bottom:12px'>", unsafe_allow_html=True
    )
    st.markdown("### 2. Refinement (Select columns & filter)")

    all_cols = safe_cols(working_df)
    sel_cols = st.multiselect("Keep Columns", all_cols, default=all_cols)
    df = working_df.copy()
    if sel_cols:
        df = df[sel_cols].copy()

    # Simple filter UI: allow multiple filters (small set)
    st.markdown("Filter rules (optional)")
    if "vb_filters" not in st.session_state:
        st.session_state["vb_filters"] = []

    # show existing filters
    if st.session_state["vb_filters"]:
        for i, f in enumerate(st.session_state["vb_filters"]):
            cols = st.columns([4, 1])
            with cols[0]:
                st.markdown(f"- **{f['col']}** {f['op']} `{f['val']}`")
            with cols[1]:
                if st.button("Remove", key=f"vb_filter_rm_{i}"):
                    st.session_state["vb_filters"].pop(i)
                    st.experimental_rerun()

    with st.expander("➕ Add Filter", expanded=False):
        filter_col = st.selectbox(
            "Column", ["--select--"] + safe_cols(df), key="vb_filter_col"
        )
        if filter_col and filter_col != "--select--":
            dtype = df[filter_col].dtype
            if np.issubdtype(dtype, np.number):
                op = st.selectbox(
                    "Operator",
                    ["between", ">", "<", ">=", "<=", "==", "!="],
                    key="vb_filter_num_op",
                )
                if op == "between":
                    vmin = float(df[filter_col].min())
                    vmax = float(df[filter_col].max())
                    rng = st.slider(
                        "Range", vmin, vmax, (vmin, vmax), key="vb_filter_num_rng"
                    )
                    add_val = (rng[0], rng[1])
                else:
                    val = st.number_input(
                        "Value",
                        value=float(
                            df[filter_col].median()
                            if not df[filter_col].isna().all()
                            else 0.0
                        ),
                        key="vb_filter_num_val",
                    )
                    add_val = val
            else:
                op = st.selectbox(
                    "Operator", ["contains", "==", "!="], key="vb_filter_str_op"
                )
                val = st.text_input("Value", key="vb_filter_str_val")
                add_val = val

            if st.button("Add Filter Rule"):
                st.session_state["vb_filters"].append(
                    {"col": filter_col, "op": op, "val": add_val}
                )
    ...
    try:
        for f in st.session_state["vb_filters"]:
            ...
            if (
                f["op"] == "between"
                and isinstance(f["val"], (list, tuple))
                and len(f["val"]) == 2
            ):
                df = df[(df[f["col"]] >= f["val"][0]) & (df[f["col"]] <= f["val"][1])]
            elif f["op"] == ">":
    ...
    st.caption(f"After filters: {len(df)} rows")
    st.dataframe(df.head(50), use_container_width=True, height=170)
    st.markdown("</div>", unsafe_allow_html=True)

    # -----------------------
    # 3. Aggregation: Multi-group & multiple aggs per column
    # -----------------------
    st.markdown(
        "<div class='glass-card' style='margin-bottom:12px'>", unsafe_allow_html=True
    )
    st.markdown("### 3. Aggregation (Group & Multiple Aggregates)")

    # group-by multi-select
    available_cols = safe_cols(df)
    group_by = st.multiselect(
        "Group By (pick 0..n columns)", available_cols, default=[]
    )

    # Aggregation rule builder: user can add many rules (col + functions)
    if "vb_agg_rules" not in st.session_state:
        st.session_state["vb_agg_rules"] = []  # each entry: {col, funcs:list}
    ...
    if st.session_state["vb_agg_rules"]:
        ...
        for i, r in enumerate(st.session_state["vb_agg_rules"]):
            ...
            if st.button("Remove", key=f"vb_agg_rm_{i}"):
                st.session_state["vb_agg_rules"].pop(i)
                st.experimental_rerun()

    with st.expander("➕ Add Aggregation Rule", expanded=False):
        numeric_cols = [
            c for c in available_cols if np.issubdtype(df[c].dtype, np.number)
        ]
        agg_col_choice = st.selectbox(
            "Agg Column", ["--select--"] + numeric_cols, key="vb_agg_col"
        )
        agg_funcs_choice = st.multiselect(
            "Functions",
        ...
        if st.button("Add Aggregation"):
            if agg_col_choice != "--select--" and agg_funcs_choice:
                st.session_state["vb_agg_rules"].append(
                    {"col": agg_col_choice, "funcs": agg_funcs_choice}
                )
    ...
    # Run aggregation if rules present and user clicks "Apply Aggregation"
    agg_preview = None
    if st.session_state["vb_agg_rules"]:
        if st.button("Apply Aggregation"):
            try:
                ...
                if group_by:
                    # groupby with multiple agg functions -> produce flat column names
                    ...
                    agg_preview = grouped.reset_index()
                else:
                    # aggregate over entire df (no grouping)
                    agg_res = df.agg({col: agg_map[col] for col in agg_map})
                    # convert to DataFrame friendly form
                    agg_preview = pd.DataFrame(agg_res).T.reset_index()
                # update df to aggregated result for charting (user intent)
                df = agg_preview.copy()
    ...
    # -----------------------
    # 4. Chart Settings + Render
    # -----------------------
    st.markdown(
        "<div class='glass-card' style='margin-bottom:12px'>", unsafe_allow_html=True
    )
    st.markdown("### 4. Chart Builder")
    ...
    def_chart = loaded_config.get("chart_type") if loaded_config else "Bar Chart"
    chart_type = st.selectbox(
        "Chart Type", chart_types, index=get_idx(chart_types, def_chart)
    )

    # X and Y choices from current df
    cur_cols = safe_cols(df)
    x_default = (
        loaded_config.get("x_axis")
        if loaded_config
        else (cur_cols[0] if cur_cols else None)
    )
    y_default = (
        loaded_config.get("y_axis")
        if loaded_config
        else (cur_cols[1] if len(cur_cols) > 1 else (cur_cols[0] if cur_cols else None))
    )

    x_axis = st.selectbox(
        "X-Axis / Label",
        ["--select--"] + cur_cols,
        index=get_idx(["--select--"] + cur_cols, x_default),
    )
    # Y should be numeric for most charts; provide all numeric columns
    numeric_cols = [c for c in cur_cols if np.issubdtype(df[c].dtype, np.number)]
    y_axis = st.selectbox(
        "Y-Axis / Values",
        ["--select--"] + numeric_cols,
        index=get_idx(["--select--"] + numeric_cols, y_default),
    )
    ...
    title = st.text_input(
        "Chart Title",
        value=(loaded_config.get("title") if loaded_config else f"{chart_type}"),
    )

    # Description (used in PDF report)
    description = st.text_area(
        "Chart Description (optional)",
        value="",
        help="This text will appear under the chart in the PDF report section.",
    )

    # Render panel
    st.markdown("#### Preview")
    if df.empty:
        st.warning("No data available for charting. Adjust joins/filters/aggregation.")
    else:
        try:
            # Validate selections
            if chart_type in ["Pie Chart", "Donut Chart"]:
                if x_axis in ["--select--", None] or y_axis in ["--select--", None]:
                    st.warning("Select X (labels) and Y (values) for pie/donut charts.")
                    fig = None
                else:
                ...
                if x_axis in ["--select--", None] or y_axis in ["--select--", None]:
                    st.warning("Choose both X and Y for this chart type.")
                    fig = None
                else:
            ...
            if fig is not None:
                ...
                chart_key = f"vb_chart_{uuid.uuid4().hex}"
                st.plotly_chart(fig, use_container_width=True, key=chart_key)

                # Actions row: Add to Composer / Single PDF / Email / Schedule
                c1, c2, c3, c4 = st.columns([1, 1, 1, 1])
                with c1:
                    if st.button("➕ Add to Composer"):
                        # store fig as PNG bytes + write temp file
                        img_bytes = None
                        try:
                        ...
                        if img_bytes:
                            try:
                                ...
                                img_path = os.path.join(
                                    tmp_dir, f"vb_chart_{uuid.uuid4().hex}.png"
                                )
                                with open(img_path, "wb") as fh:
                                    fh.write(img_bytes)
                            except Exception as e:
                                st.warning(f"Could not write temp image file: {e}")
                                img_path = None
                ...
                with c2:
                    if st.button("Download Chart PDF (single)"):
                        # single chart -> small PDF
                        try:
                            img_bytes = fig.to_image(format="png")
                        except Exception:
                            import plotly.io as pio
                ...
                with c3:
                    # Send composer as email (quick action - will include composer)
                    with st.popover("✉ Email Composer"):
                        recip = st.text_input(
                            "Recipient",
                            value="manager@aurora.io",
                            key="vb_email_recipient",
                        )
                        if st.button("Send Composer (Email)"):
                            if not st.session_state["my_charts"]:
                                st.error("Composer is empty.")
                            else:
                                # build HTML report (section per chart)
                                html_body = [
                                    "<html><head><meta charset='utf-8'><style>body{font-family:Arial;padding:20px;} img{max-width:100%;height:auto;}</style></head><body>"
                                ]
                                html_body.append(
                                    f"<h1>Composed Charts Report</h1><p>Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}</p>"
                                )
                                for item in st.session_state["my_charts"]:
                                    # prefer image_path
                                    if item.get("image_path") and os.path.exists(
                                        item["image_path"]
                                    ):
                                        img_src = "file://" + item[
                                            "image_path"
                                    ...
                                    html_body.append(
                                        "<section style='margin-bottom:30px; page-break-after:always;'>"
                                    )
                                    html_body.append(
                                        f"<h2>{item.get('title', 'Chart')}</h2>"
                                    )
                                    ...
                                    if img_src:
                                        html_body.append(
                                            f"<img src='{img_src}' alt='chart'/>"
                                        )
                                    else:
                                        html_body.append(
                                            "<p><i>[No chart image available]</i></p>"
                                        )
                                ...
                                if not success:
                                    st.error(
                                        f"PDF generation failed: {pdf_bytes_or_err}"
                                    )
                                else:
                                    pdf_bytes = pdf_bytes_or_err
                                    ...
                                    if smtp_cfg.get("host"):
                                        ...
                                        try:
                                            ...
                                            msg = EmailMessage()
                                            msg["Subject"] = f"Composed Charts Report"
                                            ...
                                            msg["To"] = recip
                                            msg.set_content(
                                                "Attached: composed charts report."
                                            )
                                            msg.add_attachment(
                                                pdf_bytes,
                                                maintype="application",
                                                subtype="pdf",
                                                filename="composed_report.pdf",
                                            )
                                            s = smtplib.SMTP(
                                                smtp_cfg["host"],
                                            ...
                                            s.send_message(msg)
                                            s.quit()
                                            st.success(f"Email sent to {recip}")
                                        except Exception as e:
                                            st.error(f"Email send failed: {e}")
                                            st.download_button(
                                                "Download PDF",
                                        ...
                                        st.info(
                                            "No SMTP configured. Download the composed report instead."
                                        )
                                        st.download_button(
                                            "Download PDF",
                                            pdf_bytes,
                ...
                with c4:
                    if st.button("📅 Schedule Composer"):
                        st.session_state["scheduler_defaults"] = {
                            "name": title or "Scheduled Visual Builder Report",
                            "dataset": primary,
                            "view_config": {
                                "chart_type": chart_type,
                                "x_axis": x_axis,
                            ...
                            "composer_ids": [
                                c.get("id") for c in st.session_state["my_charts"]
                            ],
                        }
                        st.success(
                            "Scheduler payload prepared. Go to Scheduler page to finish."
                        )
                        try:
                            st.switch_page("page_scheduler")
                        except Exception:
                            st.info(
                                "Navigate to Scheduler page to complete scheduling."
                            )

        except Exception as e:
            st.error(f"Could not render chart: {e}")
            st.info("Check axes and data types for compatibility.")

    st.markdown("</div>", unsafe_allow_html=True)

    # -----------------------
    # 5. Composer Management (list + remove + reorder minimal)
    # -----------------------
    st.markdown(
        "<div class='glass-card' style='margin-bottom:12px'>", unsafe_allow_html=True
    )
    st.markdown("### 5. Composer (selected charts)")

    if not st.session_state["my_charts"]:
        st.info("Composer is empty — add charts with ➕ Add to Composer")
    else:
        # simple list with preview and remove option
        for i, item in enumerate(
            list(st.session_state["my_charts"])
        ):  # copy for safe removal
            cols = st.columns([2, 6, 1])
            with cols[0]:
                # show thumbnail (if available)
                if item.get("image") and len(item.get("image")) > 0:
                    b64 = base64.b64encode(item["image"]).decode("utf-8")
                    st.image(f"data:image/png;base64,{b64}", width=120)
                else:
                    st.text("No preview")
            with cols[1]:
                st.markdown(f"**{item.get('title', 'Chart')}**")
                if item.get("description"):
                    st.markdown(item.get("description"))
                st.caption(
                    f"Dataset: {item.get('dataset')} • Created: {item.get('created_at', '')}"
                )
            with cols[2]:
                if st.button("Remove", key=f"composer_rm_{i}"):
                    st.session_state["my_charts"].pop(i)
                    st.experimental_rerun()

        st.markdown("---")
        # Export composer (multi-chart PDF with section headers)
        col_e1, col_e2 = st.columns(2)
        with col_e1:
            if st.button("Export Composer -> PDF"):
                # build HTML with all composer items
                html_body = [
                    "<html><head><meta charset='utf-8'><style>body{font-family:Arial;padding:20px;} img{max-width:100%;height:auto;margin:10px 0;} section{margin-bottom:30px; page-break-after:always;}</style></head><body>"
                ]
                html_body.append(
                    f"<h1>Composed Charts Report</h1><p>Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}</p>"
                )
                for item in st.session_state["my_charts"]:
                    if item.get("image_path") and os.path.exists(item["image_path"]):
                        img_src = "file://" + item["image_path"].replace("\\", "/")
                    elif item.get("image"):
                        ...
                        img_src = None
                    html_body.append("<section>")
                    html_body.append(f"<h2>{item.get('title')}</h2>")
                    if item.get("description"):
                        html_body.append(f"<p>{item.get('description')}</p>")
                    if img_src:
                        html_body.append(f"<img src='{img_src}' alt='chart'/>")
                    else:
                        html_body.append("<p><i>[No image available]</i></p>")
                    html_body.append("</section>")
                html_body.append("</body></html>")
                html = "".join(html_body)
                success, pdf_bytes_or_err = PDFEngine.generate(
